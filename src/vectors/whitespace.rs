use crate::amgix::tokenize_whitespace as aa_tokenize_whitespace;
use crate::models::VectorConfigInternal;
use crate::vectors::vector_base::{get_language_code, validate_text, VectorBase};

pub struct WhiteSpaceVector;

impl VectorBase for WhiteSpaceVector {
    fn get_sparse_vector_single(
        ...
        let (indices, values) = aa_tokenize_whitespace(
            text.to_string(),
            lang_code,
    ...
    fn get_dense_vector(
        ...
        Err("WhiteSpaceVector does not support dense vectors".to_string())
    }
}
